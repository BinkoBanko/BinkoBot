...
class Touch(commands.Cog):
    ...
    @app_commands.command(name="pin", description="Pin someone — requires chaotic, lewd, or protective vibe")
    ...
    async def pin(self, interaction: discord.Interaction, user: discord.User | None = None):
        if not await vibe_gate(
            interaction,
            ["chaotic", "lewd", "protective"],
            "You're feeling too gentle for pinning right now~ Set your vibe first!",
        ):
            return
